const { handleServerError } = require("../controllers/generalCRUDController");
const {
  Majors,
  PlanCourses,
  Courses,
...
const studentService = {
  ...
  getSemesterType: async function () {
    try {
      const CurrentSemester = await Calendar.findOne({
        attributes: ["Semester_Type"],
      });
  ...
  countPostponedSemesters: async function (student) {
    try {
      const postponeCount = await PostponeRequest.findOne({
        where: { Student_ID: student.Student_ID },
        attributes: ["No_of_Semesters"],
      });
  ...
  checkFirstSemester: async function (student) {
    try {
      ...
      if (
        studentDetails.Acceptance_Year == currentCalendar.Academic_Year &&
        studentDetails.Acceptance_Semester == currentCalendar.Semester_Type
      ) {
  ...
  isGraduate: async function (student) {
    ...
    if (Semester_Type == 3) {
      if (RemainingHours <= 10) return true;
      else return false;
    } else {
      if (RemainingHours <= 18) return true;
      else return false;
    }
  },
  ...
  isPreGraduate: async function (student) {
    ...
    if (Semester_Type == 3 || Semester_Type == 2) {
      if (RemainingHours <= 28) return true;
      else return false;
    } else {
      if (RemainingHours <= 36) return true;
      else return false;
    }
  },
  ...
  checkCourseConflicts: async function (studentID, orgCourseID) {
    try {
      ...
      for (const enrollment of enrolledCoursesSchedule) {
        ...
        if (enrolledCourse.length == 0) continue; // Skip if no schedule is attached
  ...
  canPostponeSemester: async function (student) {
    const currentCalendar = await this.getCurrentAcademicYearAndSemester();
    const postponeCount = await this.countPostponedSemesters(student);
    const studentCurrentSemesterCount =
      await this.getStudentCurrentSemesterCount(student);
  ...
  canIncreaseAcademicLoad: async function (student) {
    ...
    if (Semester_Type == 3) {
      ...
      if (studentCurrentCH == 18)
        return { eligible: true, AllowedHours: RemainingHours };
    }
